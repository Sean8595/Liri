...
function heyLiri() {
  switch (command) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          if (response.data.length > 0 && Array.isArray(response.data)) {
            ...
            console.log("No bands on tour by that name");
                    ...
                    fs.appendFile("log.txt", JSON.stringify(response), function(err){
                if (err){
                    console.log(err)
                }
                else {
    ...
    case "spotify-this-song":
      ...
      spotify.search({ type: "track", query: searchResult }, function(
        err,
        data
      ) {
        if (err) {
        ...
        for (var i = 0; i < 1; i++) {
          ...
          console.log(
            "Get a sample of that song at " + data.tracks.items[i].href
          );
        }
        addData(JSON.stringify(command))
        addData(JSON.stringify(data.tracks.items[i].artists[i].name))
        addData(JSON.stringify(data.tracks.items[i].name))
      });
      break;
    ...
    case "movie-this":
      ...
      axios.get(queryUrl).then(function(response) {
        ...
        console.log(
          "Rotten Tomato Rating is " + response.data.Ratings[1].Value
        );
        console.log("Made in " + response.data.Country);
        console.log("Language is " + response.data.Language);
        addData(JSON.stringify(command))
        addData(JSON.stringify(response.data.Title))
      });
      break;
